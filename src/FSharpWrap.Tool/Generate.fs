...
let rec mdle mname (t: Type) (cache: NameCache): PrintExpr =
    let tname = cache.GetTypeName t
    ...
    print {
        let assemblies' =
            Seq.where
                ...
                assemblies
            |> Array.ofSeq
        let cache = NameCache assemblies'
        ...
        let namespaces =
            ...
            for t in types do
                ...
                | false, _ ->
                    let entry = Dictionary(1, TypeName.comparer)
        ...
        for KeyValue(ns, types) in namespaces do
            ...
            for KeyValue({ Name = name }, t) in types do
                match t with
                | SingleType t' -> mdle name t' cache
                | MultipleTypes dups ->
